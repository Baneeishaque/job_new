					...
					<div class="row">
						...
						<div class="column-2">
                        	...
                        	<?php
                                    ...
                                    mysql_query("UPDATE jobs.posted_notification_employee SET no=0,opened='yes'' WHERE iduser='".$iduser."';");
							...
							<div class='column-1'>";
                                    ...
                                    $result = mysql_query("SELECT DISTINCT idjob FROM jobs.messages_posted_employee WHERE iduser='".$iduser."' ORDER BY idjob DESC;");
                                    while($row = mysql_fetch_array($result))
                                    {
                                        $row_employer_id = mysql_fetch_array(mysql_query("SELECT idemployer FROM jobs.job WHERE idjob='".$row["idjob"]."';"));
                                        $row_employer = mysql_fetch_array(mysql_query("SELECT email,company_name FROM jobs.employer WHERE idemployer='".$row_employer_id["idemployer"]."';"));
                                        $employer_id = $row_employer_id["idemployer"];
                                        $email = $row_employer["email"];
                                        $company_name = $row_employer["company_name"];
                                        
                                        $result2=mysql_query("SELECT * FROM jobs.job WHERE idjob='".$row["idjob"]."';");
                                    ...
                                    if($user_id == null)
                                    {
                                    echo "<a href='apply.php?job_id=".$row2["idjob"]."'><input type='button' class='formsbtn' value='Apply'></a>&nbsp&nbsp";
                                    }
                                    else
                                    {
                                    echo "<input type='button' class='formsbtn' value='Apply' disabled style='color:#757975;'>";
                                    }
