			...
			<section class="philosophy">
				...
				<div class="container">
					...
					<div class="row">
						...
						<div class="column-2">
                                                    <h2>Resume Asked<span class="line"><span></span></span></h2>
                        	...
                        	<?php
                                    ...
                                    mysql_query("UPDATE jobs.resume_notification_employee SET no=0,opened='yes' WHERE iduser='".$iduser."';");
                                    
                                    $result = mysql_query("SELECT * FROM jobs.resume_notification_info WHERE iduser='".$iduser."';");
                                    echo "<table >";
                                    while ($row = mysql_fetch_array($result))
                                    {
                                        $idemployer = $row["idemployer"];
                                        $row_company = mysql_fetch_array(mysql_query("SELECT email,company_name FROM jobs.employer WHERE idemployer='".$idemployer."';"));
                                        
                                        echo "<tr><td><a href='company_profile.php?company=".$row_company["email"]."' style='text-transform: uppercase'>".$row_company["company_name"]."</a></td><td><a href='grant_resume.php?idemployer=".$idemployer."&iduser=".$iduser."'><input type='button' class='formsbtn' value='Grant&#13;&#10;Resume' style='width:75px; height:75px;'></a></td></tr>";
                                    }
                                   echo "</table>";
