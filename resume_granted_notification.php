					...
					<div class="row">
						...
						<div class="column-2">
                        	...
                        	<?php
                                    ...
                                    mysql_query("UPDATE jobs.resume_granted_notification SET no=0,opened='yes' WHERE idemployer='".$idemployer."';");
                                    
                                    $result = mysql_query("SELECT * FROM jobs.resume_granted_info WHERE idemployer='".$idemployer."';");
                                    echo "<table >";
                                    while ($row = mysql_fetch_array($result))
                                    {
                                        $iduser = $row["iduser"];
                                        $row_user = mysql_fetch_array(mysql_query("SELECT email,name,resume FROM jobs.employee WHERE iduser='".$iduser."';"));
                                        
                                        echo "<tr><td><a href='http://localhost/job_new/resume/".$row_user["resume"]."' style='text-transform: uppercase' target='_blank'>".$row_user["name"]."</a></td></tr>";
                                    }
                                   echo "</table>";
